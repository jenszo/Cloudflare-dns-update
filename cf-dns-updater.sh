...
_cf_zone_id=$CF_ZONE_ID
_cf_dns_record_id=$CF_DNS_RECORD_ID
_cfg_dns_domain=$CF_DNS_DOMAIN
_cf_api_token=$CF_API_TOKEN
...
while true; do
  ...
  [[ "$_actual_ip" != "$_current_ip" ]] && {
    ...
    _response=$(curl -s -X PUT https://api.cloudflare.com/client/v4/zones/${_cf_zone_id}/dns_records/${_cf_dns_record_id} -H "Content-Type: application/json" -H "Authorization: Bearer ${_cf_api_token}" -H "cache-control: no-cache" -d "{\"type\" : \"A\", \"name\" : \"${_cfg_dns_domain}\", \"content\" : \"${_current_ip}\" }")
